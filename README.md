...
4. **Repairs the module configs** (`fix-config-warnings.py --no-restart`, skip with
   ...
   `ethereal_bazaar.conf`, `spellbook.conf`, ... activated). An activated config keeps the
   values of the fork, and every key where the `.conf.dist` ships a different value than the
...
| Characters no longer learn their class abilities automatically (no new spells/ranks/talents on level up) | **Not caused by this deployment** – it is a deliberate change in the fork: `mod-spellbook` restores the **Books of Ascension** as class trainers and ships `AscensionCompat.AutoProgression = 0` (config file **and** code default) in `mod-ascension-compat`: abilities are bought from the books instead of being granted on level up (`AscensionCompat.cpp`: "Switched off, nothing is granted here and the player earns them another way"). Back to the old behaviour: set `AscensionCompat.AutoProgression = 1` in `env/dist/etc/modules/mod_ascension_compat.conf` and run `cd /opt/azerothcore && docker compose restart ac-worldserver`. `fix-config-warnings.py` never overwrites a defined key, and `--keep-code-defaults` protects it when the config is (re)activated. |
